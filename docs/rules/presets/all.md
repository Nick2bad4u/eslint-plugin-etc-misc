...
Use this preset when you want every current, non-deprecated rule from
`eslint-plugin-etc-misc` with metadata-derived severities.
...
- Every non-deprecated rule under `src/rules/*.ts`.
...
lint enforcement. `all` is best for teams that prefer maximum current-rule
coverage and can commit to rule-by-rule tuning where needed. Use
`allWithDeprecated` only while migrating away from deprecated rules.
