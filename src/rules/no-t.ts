...
const rule: ReturnType<typeof ruleCreator<typeof defaultOptions, MessageIds>> =
  ...
  create: (context) => {
    ...
    return {
      "TSTypeParameter > Identifier[name=/^.$/]": (node: Readonly<es.Identifier>) => {
        context.report({
          data: { name: node.name },
          messageId: "forbidden",
          node,
      ...
      "TSTypeParameter > Identifier[name=/^.{2,}$/]": (node: Readonly<es.Identifier>) => {
        const { name } = node;
  ...
  meta: {
    ...
    schema: [
      {
        description: "Configuration for enforced generic type parameter prefix.",
        properties: {
          prefix: {
            description: "Required prefix for type parameter names longer than one character.",
            type: "string",
          },
        },
        type: "object",
